...
app.post('/api/review/consolidate', async (request, response) => {
  try {
    const { proposalOutput, reviews } = request.body || {};
    const result = await consolidateReviews(proposalOutput || {}, reviews || {});
    response.json(result);
  } catch (error) {
    response.status(500).json({ error: 'Consolidation failed.', detail: error.message });
  }
});

app.post('/api/review/cs-academic', async (request, response) => {
  try {
    const { proposalOutput } = request.body || {};
    const result = await reviewCsAcademic(proposalOutput || {});
    response.json(result);
  } catch (error) {
    response.status(500).json({ error: 'CS academic review failed.', detail: error.message });
  }
});

app.post('/api/review/consistency', async (request, response) => {
  try {
    const { proposalOutput } = request.body || {};
    const result = await reviewConsistency(proposalOutput || {});
    response.json(result);
  } catch (error) {
    response.status(500).json({ error: 'Consistency review failed.', detail: error.message });
  }
});

app.post('/api/review/methodology', async (request, response) => {
  try {
    const { proposalOutput } = request.body || {};
    const result = await reviewMethodology(proposalOutput || {});
    response.json(result);
  } catch (error) {
    response.status(500).json({ error: 'Methodology review failed.', detail: error.message });
  }
});
...
app.post('/api/review/completeness', async (request, response) => {
  try {
    const { proposalOutput } = request.body || {};
    const result = await reviewCompleteness(proposalOutput || {});
    response.json(result);
  } catch (error) {
    response.status(500).json({ error: 'Completeness review failed.', detail: error.message });
  }
});
