...
Full-stack proposal-writing agent app: React + Vite frontend proxied to an Express API backend. The app is built around a 7-step guided stepper that feeds the "Research Proposal Draft" output section, followed by a multi-agent Review Dashboard.

### 7-step guided stepper
...
3. **Design** (`DesignModal`) — system/architecture design description and an optional diagram image (JPEG/PNG/etc.). "Enhance with AI" calls `POST /api/research/enhance-design` → `enhanceDesignDescription()`. Uploading an image stores it as a base64 data URL in `proposalOutput.design_image`; the figure caption defaults to `"System Architecture Review"` and is always editable. Saved data populates `proposalOutput.design_description`, `design_image`, and `design_image_caption`.
4. **Methodology** (`MethodologyModal`) — research type, data source, tools (tag input), experiment description, contributions. "Generate Methodology" calls `POST /api/research/generate-methodology` → `generateMethodology()`.
5. **Timeline** (`TimelineModal`) — duration (weeks), team size, budget, activity list. No LLM button — the activity list is generated client-side by `defaultActivitiesForDuration(weeks)` on first open. Changing the duration regenerates the default list unless the user has already edited activities manually. On reopen, saved activities are restored. `POST /api/research/generate-timeline` → `generateTimeline()` still exists on the server but is no longer called by the frontend.
6. **Risks & Mitigation** (`RisksModal`) — risk category, description, likelihood/impact, mitigation. AI actions call `/api/research/structure-risk` and `/api/research/suggest-mitigation`. Multiple risks can be saved in a list.
7. **References** (`ReferencesModal`) — DOI lookup via `POST /api/research/fetch-doi` → `fetchDoiReference()` which first hits the CrossRef public API (`api.crossref.org`) then falls back to an LLM call. References can also be typed manually.
...
- **Clear** — resets `proposalOutput` to empty, clears all `completedSteps`, resets `projectDetails` / `researchProblemData` / `designData` / `methodologyData` / `timelineActivities` / `risksData` / `referencesData` to their initial defaults, and removes the localStorage entry.
...
- **Review Checklist** — opens a modal summarising 7-step completion status and all proposal section statuses

### Key files

| File | Role |
|---|---|
| `src/App.jsx` | Entire frontend — all state, 7 modal components (including `DesignModal`), `ProposalStepper`, `GenerateFormatPopup`, `FixButton`, `CorrectionPanel` (retained but unused in JSX), and the main App component. No sub-files. |
| `server/index.js` | Express server; API routes covering agent workflow, all 7 stepper section helpers, DOI lookup, LaTeX assembly, PDF export, 6 review agents, auto-fix, and proposal correction |
...
| `server/claudeRefine.js` | All per-section LLM helpers called by the 7-step modal popups, all 6 review agent functions, `autoFixField`, and `correctFromReview`; respects `MOCK_LLM=true`; `fetchDoiReference` hits CrossRef first |
| `server/pdfExport.js` | Converts LaTeX to PDF via `tectonic` CLI; accepts `images` dict (filename → base64 data URL), writes them to the temp dir before compilation; only sanitizes `\includegraphics` for files not in the images dict |
...
`MOCK_LLM=true` short-circuits every exported function in `claudeRefine.js` to return a static `MOCK` object (hard-coded network-ops themed sample data). The `fetchDoiReference` function always tries CrossRef first regardless of `MOCK_LLM`. The `MOCK` object includes entries for all 6 review agents (`completeness`, `quality`, `methodology`, `consistency`, `csReview`, `consolidation`), the review dashboard (`review`), the plain-text methodology generator (`methodologyText`), the design description enhancer (`designDescription`), and six agent-specific correction objects (`correctionsCompleteness`, `correctionsQuality`, `correctionsMethodology`, `correctionsConsistency`, `correctionsCs`, `correctionsConsolidation`). The `MOCK_CORRECTIONS_BY_AGENT` map selects the right mock correction object based on the `agentName` string passed to `correctFromReview()`.

### Frontend state

The main `App` component holds all state. Key state buckets:
- `projectDetails` / `researchProblemData` / `designData` / `methodologyData` / `timelineActivities` / `risksData` / `referencesData` — per-modal data. `designData` holds `{ description, design_image, design_image_caption }` and is `null` until the Design step is saved. `timelineActivities` starts as `[]`; `defaultActivitiesForDuration(weeks)` generates the initial list client-side when the modal opens with no saved data.
- `proposalOutput` — flat object with fields that populate the "Research Proposal Draft" section: `research_title`, `objective`, `problem_statement`, `hypothesis`, `motivation`, `design_description`, `design_image` (base64 data URL or `null`), `design_image_caption` (defaults to `"System Architecture Review"`), `methodology_text`, `data_source`, `tools`, `contributions`, `timeline_budget` / `timeline_structured`, `risks_mitigation` / `risks_structured`, `references`. The proposal title heading above the draft reads `proposalOutput.research_title` first (inline-editable), falling back to `projectDetails.research_title` (set via the modal).
- `completedSteps` — `{ projectDetails, researchProblem, design, methodology, timeline, risks, references }` booleans driving stepper appearance
...
- **Sections in order**: Introduction & Objective → Problem Statement → Motivation → Hypothesis → **System Design** → Methodology → Expected Contributions → Timeline → Risks and Mitigation → References
- **System Design**: `buildDesignText()` injects a `Figure~\ref{fig:design}` cross-reference after the first sentence of the description (only when an image was uploaded). The figure uses `[htbp]` placement, fills `\textwidth`, is framed by thin navy rules above and below, and has a bold-navy italic caption via `\captionsetup`. Caption defaults to `"System Architecture Review"` if not overridden.
...
- Packages used: `xcolor`, `colortbl`, `tabularx`, `booktabs`, `multicol`, `fancyhdr`, `parskip`, `titlesec`, `array`, `setspace`, `graphicx`, `caption`

### PDF compilation

`pdfExport.js` requires `tectonic` on `PATH`. It writes LaTeX to a temp directory, writes any supplied `images` (filename → base64 data URL) as binary files alongside the `.tex`, runs `tectonic`, reads back the PDF, and cleans up. `\includegraphics` references for known image filenames are preserved; all others are replaced with a text placeholder. If `tectonic` is not installed, `POST /api/export/pdf` returns a 500 and the UI shows a hint to download the LaTeX source instead.

`POST /api/export/pdf` accepts `{ proposalLatex, title, images }` where `images` is an optional object mapping filenames to base64 data URLs (e.g. `{ "design-figure.png": "data:image/png;base64,..." }`). `GenerateFormatPopup` and `handleExportLatex` both pass `proposalOutput.design_image` under the key `"design-figure.png"` when present.
